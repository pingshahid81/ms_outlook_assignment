...
@class OutLookCalendarViewCell;

@protocol OutLookCalendarViewCellDelegate <NSObject>
 ...
 *  Will be called only if the delegate returns YES for `- (BOOL)simpleCalendarViewCell:(OutLookCalendarViewCell *)cell shouldUseCustomColorsForDate:(NSDate *)date;`
 *
 *  @param cell the current cell
 *  @param date the date associated with the cell
 *
...
/**
 *  The `OutLookCalendarViewCell` class displays a day in the calendar.
 */
@interface OutLookCalendarViewCell : UICollectionViewCell

/**
 *  The delegate of the cell.
 *  Must adopt the `OutLookCalendarViewCellDelegate` protocol.
 *
 *  @see OutLookCalendarViewCellDelegate
 */
@property (nonatomic, weak) id<OutLookCalendarViewCellDelegate> delegate;
